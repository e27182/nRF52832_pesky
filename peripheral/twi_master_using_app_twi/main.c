...
uint8_t const mpu_pwr_mgmt_1_reg_addr = MPU_PWR_MGMT_1_REG;
uint8_t const mpu_who_am_i_reg_addr = MPU_WHO_AM_I_REG;

#define BUFFER_SIZE  3
static uint8_t m_buffer[BUFFER_SIZE] = { BIT_RESET, 0x00, 0x00 };

static app_twi_transfer_t const mpu_init_transfers[] = 
{
    APP_TWI_WRITE(MPU_I2C_ADDRESS, &mpu_pwr_mgmt_1_reg_addr, 1, APP_TWI_NO_STOP),
    APP_TWI_WRITE(MPU_I2C_ADDRESS, &m_buffer[0], 1, 0),

    APP_TWI_WRITE(MPU_I2C_ADDRESS, &mpu_pwr_mgmt_1_reg_addr, 1, APP_TWI_NO_STOP),
    APP_TWI_WRITE(MPU_I2C_ADDRESS, &m_buffer[1], 1, 0),
};
...
static void read(void)
{
    ...
    static app_twi_transfer_t const transfers[] =
    {
        ...
        APP_TWI_READ (MPU_I2C_ADDRESS, &m_buffer[2], 1, 0)
    };
...
int main(void)
{
    ...
    // Initialize sensors.
    APP_ERROR_CHECK(app_twi_perform(&m_app_twi, &mpu_init_transfers[0], 2, NULL));
    nrf_delay_ms(100);
    APP_ERROR_CHECK(app_twi_perform(&m_app_twi, &mpu_init_transfers[2], 2, NULL));
